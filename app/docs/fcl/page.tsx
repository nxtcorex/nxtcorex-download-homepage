...
export default function FCLPage() {
  ...
  async function fetchReleases() {
    try {
      const res = await fetch('https://api.github.com/repos/FCL-Team/FoldCraftLauncher/releases?per_page=20')
  ...
  function getDownloadUrl(version: string, arch: string): string {
    if (version === 'latest') {
      return `https://cdn1.download.anycast.ren/dl/fcl/latest/FCL-release-${arch}.apk`
    }
    return `https://cdn1.download.anycast.ren/dl/fcl/${version}/FCL-release-${version}-${arch}.apk`
  }
  ...
  const archOptions = ['all', 'arm64-v8a', 'armeabi-v7a', 'x86', 'x86_64']
        ...
        <div className="max-w-4xl mx-auto">
            ...
            <div className="space-y-4">
              <div className="bg-brand-bg border border-white/[0.05] rounded p-4">
                ...
                <div>
                  ...
                  <a
                    href="https://cdn1.download.anycast.ren/dl/fcl/1.2.3.1/FCL-release-1.2.3.1-all.apk"
                    className="text-brand-orange text-xs font-mono break-all hover:underline block"
                    target="_blank"
                    rel="noopener noreferrer"
                  >
                    https://cdn1.download.anycast.ren/dl/fcl/1.2.3.1/FCL-release-1.2.3.1-all.apk
                  </a>
                </div>
              </div>
              ...
              <div className="bg-brand-bg border border-white/[0.05] rounded p-4">
                ...
                <div>
                  <span className="text-brand-orange text-xs block mb-1">加速地址:</span>
                  <a
                    href="https://cdn1.download.anycast.ren/dl/fcl/1.2.3.1/FCL-release-1.2.3.1-arm64-v8a.apk"
                    className="text-brand-orange text-xs font-mono break-all hover:underline block"
                    target="_blank"
                    rel="noopener noreferrer"
                  >
                    https://cdn1.download.anycast.ren/dl/fcl/1.2.3.1/FCL-release-1.2.3.1-arm64-v8a.apk
                  </a>
                </div>
              </div>
            </div>
          ...
          <div className="bg-brand-bg-secondary border border-white/[0.07] rounded-lg p-6">
            ...
            <div className="space-y-3">
              {archOptions.map((arch) => (
                <div key={arch} className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                  <span className="text-brand-text-secondary text-sm min-w-[100px]">{archLabels[arch]}:</span>
                  <a
                    href={`https://cdn1.download.anycast.ren/dl/fcl/latest/FCL-release-${arch}.apk`}
                    className="text-brand-orange text-sm font-mono hover:underline break-all"
                    target="_blank"
                    rel="noopener noreferrer"
                  >
                    https://cdn1.download.anycast.ren/dl/fcl/latest/FCL-release-{arch}.apk
                  </a>
                </div>
              ))}
            </div>
